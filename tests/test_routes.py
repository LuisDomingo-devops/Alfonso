...
Cubre los endpoints existentes y los nuevos de wake word por upload.
"""
import pytest
...
def test_tools_endpoint():
    ...
    assert "detect_wake_word_in_audio" in body["tools"]
...
def test_audio_wakeword_endpoint_with_mocked_tool(monkeypatch):
    ...
    with TestClient(app) as client:
        response = client.post("/audio/wakeword", json={"keyword": "alfonso"})
...
def test_audio_wakeword_upload_endpoint_with_mocked_tool(monkeypatch):
    """Nuevo endpoint: cliente sube audio, servidor detecta wake word."""
    async def fake_detect(content=b"", filename="audio.wav", keyword="alfonso", model="small"):
        return {"status": "ok", "wake_word_detected": True, "text": "hola alfonso", "keyword": "alfonso"}

    monkeypatch.setattr(
        routes, "get_tool",
        lambda name: fake_detect if name == "detect_wake_word_in_audio" else None,
    )

    with TestClient(app) as client:
        response = client.post(
            "/audio/wakeword/upload",
            files={"file": ("chunk.wav", b"RIFF....", "audio/wav")},
            data={"keyword": "alfonso", "model": "small"},
        )
    ...
    assert body["status"] == "success"
    assert body["result"]["wake_word_detected"] is True
    assert body["result"]["keyword"] == "alfonso"


def test_audio_wakeword_upload_not_detected(monkeypatch):
    """Wake word no encontrada en el audio subido."""
    async def fake_detect(content=b"", filename="audio.wav", keyword="alfonso", model="small"):
        return {"status": "ok", "wake_word_detected": False, "text": "hola mundo", "keyword": "alfonso"}
    ...
    with TestClient(app) as client:
        response = client.post(
            "/audio/wakeword/upload",
            files={"file": ("chunk.wav", b"RIFF....", "audio/wav")},
            data={"keyword": "alfonso"},
        )

    assert response.status_code == 200
    body = response.json()
    assert body["status"] == "success"
    assert body["result"]["wake_word_detected"] is False
